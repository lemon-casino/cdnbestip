...
class Config:
    ...
    quantity: int = 0
    schedule_interval: int | None = None  # Repeat the workflow every N minutes
    ...
    def _validate_speed_settings(self) -> None:
        ...
        if self.schedule_interval is not None and self.schedule_interval <= 0:
            raise ConfigurationError("Schedule interval must be greater than 0 minutes")
...
def load_config_from_env() -> Config:
    ...
    schedule_env = os.getenv("CDNBESTIP_SCHEDULE") or os.getenv("CDNBESTIP_INTERVAL")
    if schedule_env:
        try:
            config.schedule_interval = int(schedule_env)
        except ValueError:
            pass
...
def load_config(args) -> Config:
    ...
    if hasattr(args, "schedule_interval") and args.schedule_interval is not None:
        cli_overrides["schedule_interval"] = args.schedule_interval
...
def merge_config(base_config: Config, **overrides) -> Config:
    ...
    config_dict = {
        ...
        "schedule_interval": overrides.get("schedule_interval", base_config.schedule_interval),
...
def load_config_from_args(args) -> Config:
    ...
    config.schedule_interval = args_dict.get("schedule_interval")
